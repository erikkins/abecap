...
- BUILD SEQUENCE (nothing built yet): #1 state model + entitlement choke point → #2 tier-aware serving (proof-only free payload vs realtime paid; graceful drop-to-free not 403) → then register→free, QA harness, upgrade/offer copy, drip, email-capture, /should-i-sell+/momentum copy. Everything gates off #1-2. AWAIT Erik go to start building backbone.
