...
- Answer to "maximize customer output": launch-timing risk real + safeguard inert. LEVERS: (1) WARM-START the brake, (2) tranche customer entry.
- ✅ ERIK APPROVED warm-start "as long as we run it off walk-forward data." My recommended plan (told him, awaiting confirm on the backfill part): seed bk_eq_hist from WF breakout equity at (a) GENESIS, (b) RE-BASE (Jul-24 wipe proves this needed), AND (c) BACKFILL the current live book now (self-warms ~Aug21 anyway but cold-derived; backfill = brake engages immediately off real vol profile + makes book REPRODUCIBLE → unlocks the sweep). Caveat noted: seeded brake will trim exposure → mirror weights shift (fine, 0 subs). No-fabrication OK: brake only needs recent VOL profile, not returns.
- ⏸️ AWAITING ERIK DECISION (a/b/c) on the brake approach — Erik: "we know we'd have to cold start the real book anyway" (a real live launch has no own-history; seeding off backtest is synthetic). I split it: WF warm-start = for MARKETING/steady-state numbers (unchanged); LIVE book = 3 options: (a) cold-start honest + TRANCHE customer entry (no brake code change); (b) seed bk_eq_hist from replay_sleeve('breakout') sleeve-capture (fork drafted this, uncommitted, dormant for current book) — pragmatic but synthetic; (c) REDESIGN brake to read actual day-1 holdings' recent REAL realized vol (no backtest dep, works day-1, removes the "is seeding legit" question) — MY REC. Fork (ae2152a3b105b9d0a) HELD: nothing committed/pushed/deployed/mutated; only local uncommitted draft-b in maximizer_service.py; confirmed cold entry points (run_shadow_day else branch: genesis + old-fmt rebase) are dormant for current live book (takes if/from_state). AWAIT Erik's a/b/c pick before touching load-bearing brake. Sweep still deferred until warm+reproducible.
