...
def _canonical_numbers_block() -> str:
    """Canonical performance lines built from the SSOT (perf_numbers.PERF) so autogenerated copy
    can never drift from the site. NEVER hardcode these figures here — change them in perf_numbers."""
    p, m, b = _PERF["preserver"], _PERF["maximizer"], _PERF["benchmarks"]
    dd = lambda x: f"{abs(x):g}"
    return (
        f"- Preserver: {p['yr21']['cagr']:g}%/yr, worst drawdown {dd(p['yr21']['maxdd'])}%. "
        f"Typical 2-year stretch: +{p['typical_24mo']:g}% total.\n"
        f"- Maximizer: {m['yr21']['cagr']:g}%/yr, worst drawdown {dd(m['yr21']['maxdd'])}%. "
        f"Typical 2-year stretch: +{m['typical_24mo']:g}% total. Beats the S&P ({b['spy_21yr']['cagr']:g}%) "
        f"and raw momentum ({b['raw_mom_21yr']['cagr']:g}%) on return.\n"
        f"- The market (S&P 500): {b['spy_21yr']['cagr']:g}%/yr, worst drawdown {dd(b['spy_21yr']['maxdd'])}%. "
        f"Raw momentum: {b['raw_mom_21yr']['cagr']:g}%/yr, worst drawdown {dd(b['raw_mom_21yr']['maxdd'])}%."
    )
